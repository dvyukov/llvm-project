...
class Origin {
 public:
  ...
  Origin getNextChainedOrigin(StackTrace *stack) const {
    CHECK(Origin::isValidId(raw_id_));
    StackID prev_id;
    StackID stack_id = GetChainedOriginDepot()->Get(static_cast<StackID>(getChainedId()), &prev_id);
    if (stack)
  ...
  static Origin CreateChainedOrigin(Origin prev, StackTrace *stack) {
    ...
    u32 chained_id;
    bool inserted =
        GetChainedOriginDepot()->Put(h.id(), static_cast<StackID>(prev.raw_id()), reinterpret_cast<StackID*>(&chained_id));
