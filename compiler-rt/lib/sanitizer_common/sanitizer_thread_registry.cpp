...
ThreadContextBase::ThreadContextBase(Tid tid)
    : tid(tid), os_id(0), user_id(0),
      status(ThreadStatusInvalid), detached(false),
      thread_type(ThreadType::Regular), parent_tid(kInvalidTid), next(0) {
  name[0] = '\0';
...
void ThreadContextBase::SetCreated(uptr _user_id,
                                   bool _detached, Tid _parent_tid, void *arg) {
...
void ThreadRegistry::GetNumberOfThreads(uptr *total, uptr *running,
  ...
  if (total) *total = static_cast<uptr>(threads_.size());
