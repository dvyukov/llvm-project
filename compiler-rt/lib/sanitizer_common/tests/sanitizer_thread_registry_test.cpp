...
static void CheckThreadQuantity(ThreadRegistry *registry, uptr exp_total,
                                uptr exp_running) {
  EXPECT_EQ(exp_total, registry->TotalThreads());
  EXPECT_EQ(exp_running, registry->RunningThreads());
}
...
TEST(SanitizerCommon, ThreadRegistryTest) {
  ...
  CheckThreadQuantity(&registry, 11, 1);
  // Start some of them.
  for (u32 i = 1; i <= 5; i++) {
    registry.StartThread(i, 0, ThreadType::Regular, 0);
  }
  CheckThreadQuantity(&registry, 11, 6);
  ...
  CheckThreadQuantity(&registry, 16, 6);
  ...
  CheckThreadQuantity(&registry, 16, 1);
