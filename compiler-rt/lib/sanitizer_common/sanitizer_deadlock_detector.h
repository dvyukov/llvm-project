...
class DeadlockDetector {
 public:
  ...
  void onLockAfter(DeadlockDetectorTLS<BV> *dtls, uptr cur_node, StackID stk = kInvalidStackID) {
    ensureCurrentEpoch(dtls);
    uptr cur_idx = nodeToIndex(cur_node);
  ...
  bool findEdge(uptr from_node, uptr to_node, StackID *stk_from, StackID *stk_to,
                Tid *unique_tid) {
  ...
  bool onLock(DeadlockDetectorTLS<BV> *dtls, uptr cur_node, StackID stk = kInvalidStackID) {
    ensureCurrentEpoch(dtls);
  ...
  bool onTryLock(DeadlockDetectorTLS<BV> *dtls, uptr cur_node, StackID stk = kInvalidStackID) {
    ensureCurrentEpoch(dtls);
    uptr cur_idx = nodeToIndex(cur_node);
    dtls->addLock(cur_idx, current_epoch_, stk);
    return false;
  ...
  bool onFirstLock(DeadlockDetectorTLS<BV> *dtls, uptr node, StackID stk = kInvalidStackID) {
  ...
  bool onLockFast(DeadlockDetectorTLS<BV> *dtls, uptr node, StackID stk = kInvalidStackID) {
