...
// The table defines a static mutex hierarchy (what mutexes can be locked
// under what mutexes). This table is checked to be acyclic and then
...
struct InternalDeadlockDetector {
  ...
  void CheckNoLocks() {
    for (int i = 0; i < mutex_count; i++) CHECK_EQ(locked[i].recursion, 0);
  }
