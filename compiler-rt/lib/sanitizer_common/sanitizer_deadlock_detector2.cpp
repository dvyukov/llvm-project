...
struct Link {
  ...
  explicit Link(u32 id = 0, u32 seq = 0, Tid tid = kInvalidTid, StackID s0 = kInvalidStackID, StackID s1 = kInvalidStackID)
      : id(id)
      , seq(seq)
      , tid(tid)
      , stk0(s0)
...
void DD::Report(DDPhysicalThread *pt, DDLogicalThread *lt, int npath) {
  ...
  for (int i = 0; i < npath; i++) {
    ...
    rep->loop[i].stk[0] = flags.second_deadlock_stack ? link->stk0 : kInvalidStackID;
