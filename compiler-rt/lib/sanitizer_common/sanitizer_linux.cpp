...
void Semaphore::Wait() {
  ...
  for (;;) {
    if (count == 0) {
      internal_syscall(SYSCALL(futex), (uptr)&count_, FUTEX_WAIT_PRIVATE, 0, 0, 0, 0);
      count = atomic_load(&count_, memory_order_acquire);
      continue;
    }
    if (atomic_compare_exchange_weak(&count_, &count, count - 1, memory_order_acquire))
      break;
  }
}
...
void Semaphore::Post(u32 count) {
  ...
  internal_syscall(SYSCALL(futex), (uptr)&count_, FUTEX_WAKE_PRIVATE, count, 0, 0, 0);
}
