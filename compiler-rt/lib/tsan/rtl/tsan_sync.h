...
// SyncVar is a decriptor of app synchronization objects (mutexes and atomic variables).
struct SyncVar {
  SyncVar();
...
// MetaMap maps app addresses to heap block (MBlock) and sync var (SyncVar) descriptors.
// It uses 1/2 direct shadow, see tsan_platform.h for the mapping.
class MetaMap {
 public:
  MetaMap();
