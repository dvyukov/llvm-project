...
class Shadow {
 public:
  ...
  void SetAccess(u32 addr, u32 sizeLog, bool isRead, bool isAtomic, bool isFreed) {
    //DCHECK_EQ(raw_ & 0xff, 0);
    Sid sid0 = sid_;
    (void)sid0;
    u16 epoch0 = epoch_;
    (void)epoch0;
    raw_ |= (isAtomic << 31) | (isRead << 30) | (isFreed << 29) | ((((1u << (1u << sizeLog)) - 1) << (addr & 0x7)) & 0xff);
    //if (size() != 1 << sizeLog)
...
/*
  ...
  bool Intersect(const Shadow cur, const Shadow old) {
    return cur.access_ & old.access_;
  }
*/
  static ALWAYS_INLINE
  bool AddrSizeEqual(const Shadow cur, const Shadow old) {
    return cur.access_ == old.access_;
  }

  static ALWAYS_INLINE
  bool AddrSizeEqualNotFreed(const Shadow cur, const Shadow old) {
    DCHECK(!cur.IsFreed());
    bool res = ((cur.raw_ ^ old.raw_) & 0x200000ff) == 0;
    DCHECK_EQ(res, cur.access_ == old.access_ && cur.is_freed_ == old.is_freed_);
    return res;
  }

  static ALWAYS_INLINE
  bool TwoRangesIntersect(Shadow cur, Shadow old) {
    return cur.access_ & old.access_;
  }

  ALWAYS_INLINE u32 access() const { return access_; }
  u32 ALWAYS_INLINE addr0() const { DCHECK(access_); return __builtin_ffs(access_) - 1; }
  u32 ALWAYS_INLINE size() const { DCHECK(access_); return __builtin_popcount(access_); }
  ...
  ALWAYS_INLINE
  static bool SidsAreEqualOrBothReadsOrAtomic(Shadow cur, Shadow old, bool kIsWrite, bool kIsAtomic) {
    //!!! if we move bits to the low byte, we could do:
    // (((cur.raw_ ^ old.raw_) & 0xff00) | ((cur.raw_ & old.raw_) 0x3)) - 1 <= 2;
#if 0
    //bool res = (((cur.raw_ ^ old.raw_) & 0xff00) | (old.raw_ & ((kIsWrite ^ 1) | (kIsAtomic << 1)))) - 1 <= 2;
#else
    bool res = (cur.sid_ == old.sid_) || (old.raw_ & ((u32(kIsAtomic) << 31) | (u32(kIsWrite ^ 1) << 30)));
#endif
    DCHECK_EQ(res, SidsAreEqual(cur, old) || old.IsBothReadsOrAtomic(kIsWrite, kIsAtomic));
    return res;
  }

  bool ALWAYS_INLINE IsRWWeakerOrEqual(Shadow cur, bool kIsWrite, bool kIsAtomic) const {
    DCHECK_EQ(raw_ & 0x3f, cur.raw_ & 0x3f);
    bool res = (raw_ & 0xc0000000) >= (((u32)kIsAtomic << 31) | ((kIsWrite ^ 1) << 30));
    DCHECK_EQ(res, (IsAtomic() > kIsAtomic) || (IsAtomic() == kIsAtomic && !IsWrite() >= !kIsWrite));
    return res;
  }
