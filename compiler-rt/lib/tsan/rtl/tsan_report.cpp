...
void ReportDesc::AddMemoryAccess(uptr addr, uptr external_tag, Shadow s, Tid tid,
...
void PrintStack(const SymbolizedStack *frame) {
  ...
  const char *const kInterposedFunctionPrefix = SANITIZER_MAC ? "wrap_" : "__interceptor_";
...
void PrintReport(const ReportDesc *rep) {
  ...
  if (ReportStack *stack = ChooseSummaryStack(rep)) {
    if (SymbolizedStack *frame = SkipTsanInternalFrames(stack->frames)) {
      // ReportErrorSummary calls __sanitizer_report_error_summary.
      ExternalCallback cb;
