...
ReportScope::ReportScope()
    : registry_lock_(&ctx->thread_registry), slot_lock_(&ctx->slot_mtx),
      report_lock_(&ctx->report_mtx), error_lock_() {
}
...
void TraceReplay(TracePart* first, TracePart* last, Event* last_pos,
                 Epoch epoch, Func f) {
...
Tid RestoreTid(TracePart* first, TracePart* last, Event* last_pos,
               Epoch epoch) {
  Tid tid = kInvalidTid;
  TraceReplay(first, last, last_pos, epoch,
              [&](Tid evTid, Epoch evEpoch, Event* evp) {
                if (evEpoch == epoch)
                  tid = evTid;
              });
  return tid;
...
void RestoreStack(EventType type, Sid sid, Epoch epoch, uptr addr, uptr size,
  ...
  Tid tid = kInvalidTid;
  TraceReplay(trace->first, last_part, last_pos, epoch,
              [&](Tid evTid, Epoch evEpoch, Event* evp) {
                if (evEpoch == epoch)
                  tid = evTid;
              });
  ...
  *ptid = tid;
  ...
  TraceReplay(
      trace->first, last_part, last_pos, epoch,
      [&](Tid evTid, Epoch evEpoch, Event* evp) {
        if (evTid != tid)
          return;
        if (evp->isAccess) {
        ...
        case EventTypeRLock: {
          ...
          uptr evPC = RestorePC(ev->pc, ev->isExternalPC);
          StackID stackID =
              static_cast<StackID>((ev->stackIDHi << 16) + ev->stackIDLo);
...
void ReportRace(ThreadState* thr, RawShadow* shadow_mem, Shadow cur,
  ...
  //!!! re slot_mtx: how atomic is this? if we detect a bug, then reset happens,
  //!traces reset,
  // then we try to report and fail to restore traces
  ReportScope report_scope;

  RestoreStack(EventTypeAccessEx, s[1].sid(), s[1].epoch(), addr1, s[1].size(),
               s[1].IsRead(), s[1].IsAtomic(), s[1].IsFreed(), &tids[1],
               &traces[1], mset[1], &tags[1]);
