...
bool ContainsSameAccess(RawShadow* s, Shadow cur, int unused0, int unused1,
  ...
  for (uptr i = 0; i < kShadowCnt; i++) {
    ...
    auto masked = static_cast<RawShadow>(static_cast<u32>(old) |
                                         static_cast<u32>(kShadowRodata));
    if (masked == cur.raw())
      return true;
    if (!(typ & kAccessNoRodata) && !SANITIZER_GO) {
      if (old == kShadowRodata)
        return true;
    }
  }
  return false;
...
bool CheckRaces(ThreadState* thr, RawShadow* shadow_mem, Shadow cur,
  ...
  for (uptr idx = 0; idx < kShadowCnt; idx++) {
    ...
    if (LIKELY(old.raw() == kShadowEmpty)) {
...
bool ContainsSameAccess(RawShadow* unused0, Shadow unused1, m128 shadow,
  ...
  // Range memory accesses check kShadowRodata before calling this,
  // kShadowRodatas is not possible for free memory access
  // and Go does not use kShadowRodata.
  if (!(typ & kAccessNoRodata) && !SANITIZER_GO) {
...
void ShadowSet(RawShadow* p, RawShadow* end, RawShadow v) {
  ...
  for (; p < end; p += kShadowCnt) {
    p[0] = v;
    for (uptr i = 1; i < kShadowCnt; i++) p[i] = kShadowEmpty;
  }
#else
  m128 vv = _mm_setr_epi32(static_cast<u32>(v), static_cast<u32>(kShadowEmpty),
                           static_cast<u32>(kShadowEmpty),
                           static_cast<u32>(kShadowEmpty));
...
static void MemoryRangeSet(ThreadState* thr, uptr pc, uptr addr, uptr size,
  ...
  RawShadow* end1 =
      Min(end, reinterpret_cast<RawShadow*>(RoundUp(
                   reinterpret_cast<uptr>(p) + kPageSize / 2, kPageSize)));
  ShadowSet(p, end1, val);
  // Reset middle part.
  RawShadow* p1 = p;
...
void MemoryRangeFreed(ThreadState* thr, uptr pc, uptr addr, uptr size) {
  ...
  for (; size; size -= kShadowCell, shadow_mem += kShadowCnt) {
    ...
    StoreShadow(&shadow_mem[2], kShadowEmpty);
    StoreShadow(&shadow_mem[3], kShadowEmpty);
  }
#endif
}
...
void MemoryAccessRangeT(ThreadState* thr, uptr pc, uptr addr, uptr size) {
  ...
  if (is_read && *shadow_mem == kShadowRodata)
    return;
