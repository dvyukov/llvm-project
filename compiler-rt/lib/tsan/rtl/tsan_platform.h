...
uptr ShadowToMemImpl(RawShadow *s) {
  ...
  // same address.
  uptr p = (sp / kShadowMultiplier) ^ Mapping::kAppMemXor;
  if (p >= Mapping::kLoAppMemBeg && p < Mapping::kLoAppMemEnd &&
      MemToShadow(p) == reinterpret_cast<RawShadow *>(s))
    return p;
# ifdef TSAN_MID_APP_RANGE
  p = ((sp / kShadowMultiplier) ^ Mapping::kAppMemXor) +
      (Mapping::kMidAppMemBeg & Mapping::kAppMemMsk);
  if (p >= Mapping::kMidAppMemBeg && p < Mapping::kMidAppMemEnd &&
      MemToShadow(p) == reinterpret_cast<RawShadow *>(s))
    return p;
# endif
  return ((sp / kShadowMultiplier) ^ Mapping::kAppMemXor) | Mapping::kAppMemMsk;
#else  // #if !SANITIZER_GO
# ifndef SANITIZER_WINDOWS
  return (sp & ~Mapping::kShadowBeg) / kShadowMultiplier;
# else
  return (sp - Mapping::kShadowBeg) / kShadowMultiplier;
