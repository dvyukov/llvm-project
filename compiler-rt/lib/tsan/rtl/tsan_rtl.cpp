    ...
    for (uptr p0 = beg; p0 <= end; p0 += (end - beg) / 4) {
      for (int x = -(int)kShadowCell; x <= (int)kShadowCell; x += kShadowCell) {
        ...
        if (prev) {
          ...
          CHECK_EQ((s - prev_s) * sizeof(*s), (p - prev) * kShadowMultiplier);
...
void TraceSwitch(ThreadState* thr) {
  ...
  part->trace = trace;
  //!!! instead of storing start_stack/mset, trace corresponding events (func
  //! entry, mutex lock)?
  ...
  // Keep roughly half of parts local to the thread (not queued into the recycle
  // queue).
...
bool ContainsSameAccessSlow(RawShadow* s, RawShadow a, bool isRead) {
  ...
  for (uptr i = 0; i < kShadowCnt; i++) {
    ...
    if (isRead && old.raw() == Shadow::kRodata)
      return true;
...
ALWAYS_INLINE WARN_UNUSED_RESULT bool TraceMemoryAccess(ThreadState* thr,
  ...
  evex->isRead = !!(typ & kAccessRead);
  evex->isAtomic = !!(typ & kAccessAtomic);
...
bool CheckRaces(ThreadState* thr, RawShadow* shadow_mem, Shadow cur,
  ...
  DPrintf2(
      "#%d:  MOP: not_intersect=%V same_sid=%V both_read_or_atomic=%V "
      "race_mask=%04x\n",
      (int)cur.sid(), not_intersect, same_sid, both_read_or_atomic, race_mask);
...
#undef LOAD_EPOCH
  ...
  DPrintf2(
      "#%d:  MOP: shadow_epochs=%V thread_epochs=%V concurrent_mask=%04x\n",
...
ALWAYS_INLINE USED void MemoryAccess(ThreadState* thr, uptr pc, uptr addr,
  ...
  DPrintf2(
      ...
      (int)thr->tid, (void*)pc, (void*)addr, size, typ, shadow_mem,
...
ALWAYS_INLINE WARN_UNUSED_RESULT bool TryTraceMemoryAccessRange(
...
static void MemoryRangeSet(ThreadState* thr, uptr pc, uptr addr, uptr size,
  ...
  if (SANITIZER_WINDOWS || size < common_flags()->clear_shadow_mmap_threshold) {
    ...
    for (uptr i = 0; i < size / kShadowCell * kShadowCnt;) {
      p[i++] = val;
      for (uptr j = 1; j < kShadowCnt; j++) p[i++] = Shadow::kEmpty;
    }
  } else {
    ...
    while ((p < begin + kPageSize / kShadowSize / 2) || ((uptr)p % kPageSize)) {
      *p++ = val;
      for (uptr j = 1; j < kShadowCnt; j++) *p++ = Shadow::kEmpty;
    }
    // Reset middle part.
    RawShadow *p1 = p;
...
void MemoryResetRange(ThreadState* thr, uptr pc, uptr addr, uptr size) {
  ...
  MemoryRangeSet(thr, pc, addr1, size1, Shadow::kEmpty);
}
