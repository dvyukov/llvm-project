...
extern "C" void __tsan_dump() {
  ...
  for (auto& slot: ctx->slots) {
    ...
    Printf("  slot %u: reset_wait=%u/%u tid=%d pid=%d\n",
      (u32)slot.sid, slot.reset_wait, slot.thr ? atomic_load_relaxed(&slot.thr->reset_pending) : 0,
...
static TracePart* TracePartAlloc() {
  ...
  part->next = nullptr;
  part->events[TracePart::kSize - 1] = NopEvent; //!!! how many do we need to initialize?
  return part;
}
...
void DoReset() {
  ...
  for (auto& slot: ctx->slots) {
   ...
   for (uptr i = 0; i < ARRAY_SIZE(ctx->slots); i++) {
    ...
    slot->next = i != (ARRAY_SIZE(ctx->slots) - 1) ? &ctx->slots[i+1] : nullptr;
  }
  ...
  ctx->metamap.Reset();
  {
    ...
    for (u32 i = ctx->thread_registry.NumThreadsLocked(); i--;) {
      ...
      if (!tctx)
        continue;
      Free(tctx->sync);
      // Potentially we could purge all ThreadStatusDead threads from the registry.
      // Since we reset all shadow, they can't race with anything anymore.
      // However, their tid's can still be stored in some aux places (e.g. tid of thread that created something).
...
TidSlot* FindAttachSlotImpl(ThreadState* thr) {
  ...
  if (ctx->free_slot_head && !SlotUsable(ctx->free_slot_head)) {
    Printf("XXX: slot=%d head=%u tail=%u\n", (u32)slot->sid, (u32)ctx->free_slot_head->sid, ctx->free_slot_tail->sid);
...
TidSlot& FindAttachSlot(ThreadState* thr) {
  ...
  for (;;) {
    ...
    if (!atomic_load_relaxed(&ctx->reset_pending)) {
      //!!! handle the case when all slots are busy, but not exhausted (>256 threads), just wait
...
void Initialize(ThreadState* thr) {
  ...
  SetCheckFailedCallback(__tsan::CheckFailed);
...
void TraceSwitch(ThreadState* thr) {
  ...
  // we either need to remove false positive switches, or store some fake 0 event in this position
  if (pos + 1 < end)
    return;
#if !SANITIZER_GO
  // !!! can we still do this? should we at least rewind pos to beginning of part?
...
bool ContainsSameAccessSlow(RawShadow* s, RawShadow a, bool isRead) {
  ...
  for (uptr i = 0; i < kShadowCnt; i++) {
    ...
    if (Shadow::AddrSizeEqualNotFreed(cur, old) &&
        old.sid() == cur.sid() &&
        old.epoch() == cur.epoch() &&
...
bool TraceMemoryAccess(ThreadState *thr, uptr pc, uptr addr, uptr sizeLog, bool isRead, bool isAtomic) {
  if (!kCollectHistory)
    return true;
  EventAccess* ev;
...
static NOINLINE
void ReportRaceV(ThreadState* thr, RawShadow* shadow_mem, Shadow cur, u32 race_mask, m128 shadow) {
...
void MemoryAccess(ThreadState* thr, uptr pc, uptr addr,
                                     u32 kAccessSizeLog, bool kAccessIsWrite, //!!! change all kAccessIsWrite to isRead
                                     bool kIsAtomic) {
  ...
  if (!TraceMemoryAccess(thr, pc, addr, kAccessSizeLog, !kAccessIsWrite, kIsAtomic))
    return TraceRestartMemoryAccess<kInRuntime>(thr, pc, addr, kAccessSizeLog, kAccessIsWrite, kIsAtomic);
  ...
  const m128 no_race = _mm_or_si128(_mm_or_si128(not_intersect, same_sid), both_read_or_atomic);
  const int race_mask = _mm_movemask_epi8(_mm_cmpeq_epi32(no_race, zero));
  DPrintf2("  MOP: not_intersect=%V same_sid=%V both_read_or_atomic=%V race_mask=%04x\n", not_intersect, same_sid, both_read_or_atomic, race_mask);
...
STORE:
{
  ...
  const m128 access_read_atomic = _mm_set1_epi32(((u32)kIsAtomic << 31) | ((kAccessIsWrite ^ 1) << 30));
  const m128 rw_weaker = _mm_cmpeq_epi32(_mm_max_epu32(shadow, access_read_atomic), shadow);
...
SHARED:
  // Need to unwind this because _mm_extract_epi8/_mm_insert_epi32 indexes must be constants.
...
ALWAYS_INLINE USED void MemoryAccessImpl(ThreadState* thr, uptr addr,
  ...
  DPrintf2("    Access:%p access=0x%x"
           " is_write=%d shadow=%p {%s, %s, %s, %s}\n",
           (void*)addr,
           (int)cur.access(), kAccessIsWrite, shadow_mem,
           DumpShadow(memBuf[0], shadow_mem[0]), DumpShadow(memBuf[1], shadow_mem[1]),
           DumpShadow(memBuf[2], shadow_mem[2]), DumpShadow(memBuf[3], shadow_mem[3]));
