...
void DontNeedShadowFor(uptr addr, uptr size) {
  ReleaseMemoryPagesToOS(reinterpret_cast<uptr>(MemToShadow(addr)), reinterpret_cast<uptr>(MemToShadow(addr + size)));
}
...
ALWAYS_INLINE Shadow LoadShadow(RawShadow* p) {
  return Shadow(static_cast<RawShadow>(atomic_load((atomic_uint32_t*)p, memory_order_relaxed)));
}
...
void MemoryAccessImpl1(ThreadState* thr, uptr addr, u32 kAccessSize,
    ...
    return;
  {
    ...
    uptr index = static_cast<uptr>(atomic_load_relaxed(&thr->trace_pos)) % kShadowCnt;
...
void MemoryRangeFreed(ThreadState* thr, uptr pc, uptr addr, uptr size) {
  ...
  const m128 access = _mm_set1_epi32(static_cast<u32>(cur.raw()));
  const m128 freed = _mm_setr_epi32(static_cast<u32>(Shadow::FreedMarker()), static_cast<u32>(Shadow::Freed(cur.sid(), cur.epoch())), 0, 0);
