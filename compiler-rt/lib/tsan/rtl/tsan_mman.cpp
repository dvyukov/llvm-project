...
void OnUserAlloc(ThreadState *thr, uptr pc, uptr p, uptr sz, bool write) {
  ...
  // As the result this is not necessary synchronized with DoReset,
  // which interates over and resets all sync objects,
...
void OnUserFree(ThreadState *thr, uptr pc, uptr p, bool write) {
  CHECK_NE(p, 0);
  // FreeBlock resets all sync objects in the range
  // and must not run concurrently with DoReset which resets
  // all sync objects w/o any synchronization.
  // If we don't have a slot (very early/late in thread lifetime),
  // then we lock ctx->slot_mtx which is also locked by DoReset.
  if (!thr->slot) {
    Lock lock(&ctx->slot_mtx);
    ctx->metamap.FreeBlock(thr->proc(), p);
    return;
  }
