...
void MutexCreate(ThreadState *thr, uptr pc, uptr addr,
                 u32 flagz) NO_THREAD_SAFETY_ANALYSIS {
...
void MutexPreLock(ThreadState *thr, uptr pc, uptr addr,
                  u32 flagz) NO_THREAD_SAFETY_ANALYSIS {
...
void MutexPreReadLock(ThreadState *thr, uptr pc, uptr addr,
                      u32 flagz) NO_THREAD_SAFETY_ANALYSIS {
...
void MutexReadUnlock(ThreadState *thr, uptr pc,
                     uptr addr) NO_THREAD_SAFETY_ANALYSIS {
...
void MutexReadOrWriteUnlock(ThreadState *thr, uptr pc,
                            uptr addr) NO_THREAD_SAFETY_ANALYSIS {
...
void MutexRepair(ThreadState *thr, uptr pc,
                 uptr addr) NO_THREAD_SAFETY_ANALYSIS {
...
void MutexInvalidAccess(ThreadState *thr, uptr pc,
                        uptr addr) NO_THREAD_SAFETY_ANALYSIS {
...
void Acquire(ThreadState *thr, uptr pc, uptr addr) NO_THREAD_SAFETY_ANALYSIS {
  ...
  if (thr->ignore_sync)
    return;
  SlotLocker locker(thr);  //!!! can we move this to after the check below
  auto s = ctx->metamap.GetSyncIfExists(addr);
  if (!s || !s->clock)
    return;
  ReadLock lock(&s->mtx);
...
void ReleaseStoreAcquire(ThreadState *thr, uptr pc,
                         uptr addr) NO_THREAD_SAFETY_ANALYSIS {
...
void ReportDestroyLocked(ThreadState *thr, uptr pc, uptr addr,
  ...
  rep.typ = ReportTypeMutexDestroyLocked;
  {
    ReportScope report_scope(thr);
    Tid tid;
    if (!RestoreStack(EventTypeLock, last_lock.sid(), last_lock.epoch(), addr,
                      0, false, false, false, &tid, &trace[1], &mset))
      return;
