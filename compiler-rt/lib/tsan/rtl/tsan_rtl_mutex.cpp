...
static void ReportMutexMisuse(ThreadState *thr, uptr pc, ReportType typ, uptr addr,
...
void MutexPostLock(ThreadState *thr, uptr pc, uptr addr, u32 flagz, int rec) {
  ...
  TraceMutexLock(thr, EventType::kLock, pc, addr, creation_stack_id);
  thr->mset.AddAddr(addr, creation_stack_id, true);
...
void MutexPostReadLock(ThreadState *thr, uptr pc, uptr addr, u32 flagz) {
  ...
  TraceMutexLock(thr, EventType::kRLock, pc, addr, creation_stack_id);
  thr->mset.AddAddr(addr, creation_stack_id, false);
...
void ReportDestroyLocked(ThreadState *thr, uptr pc, uptr addr,
  ...
  rep.typ = ReportTypeMutexDestroyLocked;
  {
    ...
    if (!RestoreStack(EventType::kLock, last_lock.sid(), last_lock.epoch(), addr,
                      0, kAccessWrite, &tid, &trace[1], &mset, &tag))
      return;
    rep.AddMutex(addr, creation_stack_id);
