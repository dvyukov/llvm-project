...
TEST(MetaMap, Sync) {
  ...
  s1 = m->GetIfExists((uptr)&block[0]);
  EXPECT_EQ(s1, (SyncVar*)0);
  s2 = m->GetIfExists((uptr)&block[1]);
...
TEST(MetaMap, MoveMemory) {
  ...
  s1 = m->GetIfExists((uptr)&block1[0]);
  EXPECT_EQ(s1, (SyncVar*)0);
  s2 = m->GetIfExists((uptr)&block1[1]);
  EXPECT_EQ(s2, (SyncVar*)0);
  s1 = m->GetIfExists((uptr)&block2[0]);
  EXPECT_NE(s1, (SyncVar*)0);
  EXPECT_EQ(s1->addr, (uptr)&block2[0]);
  s2 = m->GetIfExists((uptr)&block2[1]);
  EXPECT_NE(s2, (SyncVar*)0);
